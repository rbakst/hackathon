...
public class SimulationValues {
	...
	void setValues(Scanner in){{
		  ...
		  System.out.println( "Enter percent of calls serviced between ");
		  int perc, 
		      sum = 0;
		  int j=0;
		  for (int i = 0; i < NUM_LIMITS -1; i++)
		  { j++;
		    System.out.print( "   " + (j-1) + " and "  + j  + " min. ");
		    perc = in.nextInt();
		    sum += perc;
    ...
    void checkForNewCall(){
    	...
    	if (x < 100 * arrivalRate){
    		System.out.println("a new call has arrived. generate a random service time for it");
    		int r = numGenerator.nextInt(100);
    		int serviceTime =0;
    		while (r > servicePercent[serviceTime]){
    			serviceTime++;  //add another minute
    		}
    		System.out.println("construct a new Call and add it to the queue of incoming calls");
  ...
  //--- Definition of service()
    public void service()
    {
    	System.out.println("Servicing call - busy time remaining: " + busyTimeRemaining);
